...
| 루브릭 항목 | 재사용 | 사유 |
|---|---|---|
| `DIMENSIONS` | 그대로 사용 | 치수 누락 검사는 도면 종류와 무관하게 성립한다. |
| `TOLERANCE` | 그대로 사용 | 코어·캐비티 끼워맞춤에 오히려 더 중요하다. |
| `SURFACE` | 그대로 사용 | 성형면 다듬질 지시 검사에 그대로 쓸 수 있다. |
| `GEOMETRIC` | 그대로 사용 | 형합면 평행도·직각도 검사에 적용된다. |
| `NOTES_TITLE` | 임계값 조정 | 금형 도면 표제란·부품란 항목이 다르다. |
| `MATERIAL` | 규칙 확장 | 금형강(SKD11, NAK80, STAVAX 등) 재질명 사전이 필요하다. |
| `PROJECTION_LAYOUT` | 임계값 조정 | 조립도 기준의 투상 요구가 부품도와 다르다. |
