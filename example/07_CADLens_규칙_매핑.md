...
| 루브릭 항목 | 재사용 | 사유 |
|---|---|---|
| `DIMENSIONS` | 그대로 사용 | 치수 누락 검사가 동일하게 성립한다. |
| `TOLERANCE` | 그대로 사용 | 다이·펀치 클리어런스 관리에 필수적이다. |
| `SURFACE` | 그대로 사용 | 전단면 다듬질 지시 검사에 적용된다. |
| `GEOMETRIC` | 그대로 사용 | 가이드포스트 위치도 검사에 적용된다. |
| `NOTES_TITLE` | 임계값 조정 | 금형 도면 표제란 항목이 다르다. |
| `MATERIAL` | 규칙 확장 | 공구강(SKD11, SKH51 등) 재질명 사전이 필요하다. |
| `PROJECTION_LAYOUT` | 임계값 조정 | 조립도 단면 요구가 기능사 과제와 다르다. |
