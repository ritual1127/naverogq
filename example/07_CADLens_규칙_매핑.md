...
| 루브릭 항목 | 재사용 | 사유 |
|---|---|---|
| `DIMENSIONS` | 그대로 사용 | 작업형 도면에 동일하게 적용된다. |
| `TOLERANCE` | 그대로 사용 | 끼워맞춤 검사 로직이 동일하다. |
| `SURFACE` | 그대로 사용 | 표면거칠기 검사가 동일하다. |
| `GEOMETRIC` | 그대로 사용 | 기하공차 검사가 동일하다. |
| `NOTES_TITLE` | 임계값 조정 | 표제란 요구 항목 확인이 필요하다. |
| `MATERIAL` | 그대로 사용 | 재질·열처리 검사가 동일하다. |
| `PROJECTION_LAYOUT` | 임계값 조정 | 기사 수준 과제의 투상도 요구를 재확인해야 한다. |
