...
**고칠 자리를 도면 위에 그려서 보여 줍니다.** 치수가 빠진 구멍과 중심선이 없는 원은 결과 화면의 `수정 예시` 버튼을 누르면, 도면에서 읽은 좌표 그대로 지름 치수(`Ø28.5`)와 중심 마크가 초록색으로 올라갑니다. 그림을 새로 만드는 AI 가 아니라 도면 좌표로 그리는 것이라 위치와 값이 정확하고, 서버를 다시 부르지 않아 누르는 즉시 보입니다. 표면거칠기 기호·기하공차·끼워맞춤은 어느 면과 치수에 넣을지가 설계 판단이라 그리지 않습니다.
