...
def fixtures(out_dir):
    return [
        ...
        ("외형선과 치수선이 같은 굵기",
         ...
         {"EX_LINEWEIGHT_FLAT"}),
        # 7mm 는 KS 문자 크기 호칭 안에 있는 값이라 지적하지 않는다.
        ("치수 문자가 7mm (규격 안의 크기)",
         _full(out_dir, "big_text", dim_text=7.0), set()),
        ("치수 문자가 1.5mm 로 못 읽을 크기",
         _full(out_dir, "tiny_text", dim_text=1.5), {"EX_TEXT_SIZE"}),
        ("치수 문자가 규격에 없는 4.2mm",
         _full(out_dir, "odd_text", dim_text=4.2), {"EX_TEXT_SIZE"}),
        ("거칠기 기호는 있는데 비교표가 없음",
